...
function ResponsiveDialogContent({
	...
	if (isMobile) {
		return (
			<DrawerContent
				autoFocus={autoFocus}
				className={cn("flex flex-col gap-4 px-4 pb-0", className)}
			>
				{children}
			</DrawerContent>
		)
	...
	if (isMobile) {
		return (
			<div
				...
				className={cn(
					"-mx-4 flex flex-col-reverse gap-2 border-t bg-muted/50 px-4 pt-3 pb-[calc(1.5rem+env(safe-area-inset-bottom))]",
					className,
				)}
				{...props}
			>
